...
# Cambia esta frase secreta por algo que solo tú conozcas
SECRET_KEY = "MiEmpresaSegura2026"


def validar_licencia(clave_ingresada):
    """Verifica si la clave introducida es válida y si aún no ha expirado."""
    try:
        # El formato de la clave debe ser: FECHA_HASTA_YYYYMMDD-HASH
        partes = clave_ingresada.strip().split("-")
        if len(partes) != 2:
            return False, "Formato de licencia inválido."

        fecha_str, hash_cliente = partes

        # 1. Verificar si la firma/hash es auténtica (evita que la inventen)
        hash_esperado = hashlib.sha256(
            (fecha_str + SECRET_KEY).encode()
        ).hexdigest()[:8]
        if hash_cliente.upper() != hash_esperado.upper():
            return False, "Licencia no válida o alterada."

        # 2. Verificar si la fecha de expiración ya pasó
        ...
        if fecha_actual > fecha_expiracion:
            return (
                False,
                f"La licencia expiró el {fecha_expiracion.strftime('%d/%m/%Y')}.",
            )

        return True, f"Licencia activa hasta {fecha_expiracion.strftime('%d/%m/%Y')}."
...
if not st.session_state.licencia_activa:
    st.title("🔒 Activación del Sistema")
    st.warning("Este software requiere una licencia activa para funcionar.")

    clave_input = st.text_input(
        "Ingrese su Clave de Licencia:", type="password"
    )

    if st.button("Activar Licencia"):
        es_valida, mensaje = validar_licencia(clave_input)
        if es_valida:
            st.session_state.licencia_activa = True
            st.success(mensaje)
            st.rerun()  # Recarga la página y entra al sistema
        else:
            st.error(mensaje)

    # Detiene la ejecución para que no cargue el resto de la app
    st.stop()
import streamlit as st
import json
import os
